...
let cacheWriteTimer: ReturnType<typeof requestIdleCallback> | ReturnType<typeof setTimeout> | null = null;
let pendingCacheData: { prices: Record<string, number>; fxRate: number } | null = null;
 ...
 */
export function loadCachedPrices(): Record<string, number> {
  try {
 ...
 */
export function loadCachedFxRate(): number {
  try {
 ...
 */
export function cachePrices(prices: Record<string, number>, fxRate: number): void {
  // Store pending data
  ...
  if (cacheWriteTimer) {
    if (typeof cancelIdleCallback !== 'undefined') {
      cancelIdleCallback(cacheWriteTimer as ReturnType<typeof requestIdleCallback>);
    } else {
      clearTimeout(cacheWriteTimer as ReturnType<typeof setTimeout>);
    }
  }
