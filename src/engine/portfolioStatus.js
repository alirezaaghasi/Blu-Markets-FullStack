 ...
 *
 * @param {Object} layerPct - Current layer percentages { FOUNDATION, GROWTH, UPSIDE }
 * @param {Object} targetLayerPct - Target layer percentages (optional, uses range midpoints if not provided)
 */
export function computePortfolioStatus(layerPct, targetLayerPct = null) {
  const issues = [];

  // Use target if provided, otherwise use midpoint of acceptable range
  const target = targetLayerPct || {
