...
// Provides same interface as realApi but uses Redux store data
...
// Import response types from realApi for consistency
import type {
  AuthResponse,
...
export const auth = {
  ...
  verifyOtp: async (_phone: string, code: string): Promise<AuthResponse> => {
    ...
    return {
      ...
      isNewUser: false, // In demo, user already has portfolio
    };
  },
...
export const rebalance = {
  ...
  preview: async (): Promise<RebalancePreview> => {
    ...
    return {
      mode: 'HOLDINGS_ONLY',
      before,
      after: targetLayerPct, // After rebalance, should match target
      target: targetLayerPct,
      trades: [], // Simplified - no trade details in mock
      cashDeployed: 0,
      residualDrift: 0,
...
export const loans = {
  ...
  getCapacity: async (): Promise<LoanCapacityResponse> => {
    ...
    const maxCapacity = portfolioValue * 0.25; // 25% max
  ...
  create: async (amountIrr: number, termMonths: 3 | 6): Promise<Loan> => {
    ...
    // Find best collateral (prefer GROWTH assets)
  ...
  repay: async (loanId: string, amountIrr: number): Promise<{
    ...
    // Update installments with payment
...
export const user = {
  ...
  updateSettings: async (settings: Partial<UserSettings>): Promise<UserSettings> => {
    ...
    // In demo mode, just return the settings
    return {
